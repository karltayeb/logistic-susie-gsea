...
GO-BP
* L1 = leukocyte/immune response
* L2 = protein transport/localization
* L3 = energy metabolism
* L4 = transport
* L5 = cell death
* L6 = translation
* L7 = RNA + (3 viral/sybiosis gene sets under the radar)
...
* L4 = tranport
* L5 = apoptosis
* L6 = translation/biosynthesis
* L7 = RNA + (3 viral/sybiosis gene sets under the radar)

L7 seems interesting. It mainly consists of RNA processing gene sets. However, a few gene sets like "GO:0044403	symbiont process" and "GO:0044419	interspecies interaction between organisms" appear in the top 10 (allbeit with $\alpha \approx 1e-5$). The interspecies gene sets have p-value 0, but they are "masked" by the stronger enrichments by SuSiE. 

I suppose the question is would we want a GSEA method to report this type of result? I'm tempted to say "the RNA gene sets explain away the symbiotic gene set enrichments", but that is trusting that their high rank in the gene set is meaningful despite their low alpha value.
...
### Non-specific enrichments

A lot of the cell types are enriched for the same gene sets! If we're interested in differences between different immune celltypes, as I presume we are, it would be good to be able to spot **different** enrichments across cell types.

Here we just look at all geneSets with suggestive PIP in at least one celltype and list off the PIPs in all cell types. This isn't perfect, it's possible that two celltypes both use the same "theme" but promote a different gene set as the representative. Indeed, **everything** is marginally enriched.

```{r}
hist(pbmc.geneset.summary$pValueHypergeometric)

pbmc.geneset.summary %>%
  arrange(desc(pip)) %>%
  group_by(geneSet, db) %>% 
  filter(max(pip) > 0.2) %>%
  select(description, pip, pValueHypergeometric, celltype) %>%
  chop(c(pip, pValueHypergeometric, celltype)) %>%
  arrange(db) %>%
