fit_affinity_prop_ora = function(ora, X, y){
  ...
  apRes <- WebGestaltR::affinityPropagation(idsInSet, minusLogP)
  return(apRes)
}

fit_weighted_set_cover_ora = function(ora, X, y, n=10){
  sig.ora  <- ora %>% filter(pHypergeometric < 1e-2)
  X.sig <- X[, sig.ora$geneSet]
  X.y <- X.sig[(y == 1),]
  ...
  minusLogP <- -log(ora[match(names(idsInSet), ora$geneSet),]$pHypergeometric)
  minusLogP[minusLogP == Inf] <- -log(.Machine$double.eps)
  wscRes <- WebGestaltR::weightedSetCover(idsInSet, 1 / minusLogP, n, 1)
  return(wscRes)
}
...
run_enrichment2 = function(db, experiment, thresh, genesets, data){
  message(paste0('Fitting logistic susie...',
                 '\n\tExperiment = ', experiment,
                 '\n\tDatabase = ', db,
                 '\n\tthresh = ', thresh))

  bin.data <- gseasusie::prep_binary_data(genesets[[db]], data[[experiment]], thresh=thresh)
  X <- bin.data$X
  y <- bin.data$y
...
# use .tresh instead of .propgenes
enrichment_target_factory2 <- function(prefix, data_loader, .genesets, .thresh, .experiments=NULL){
  # load data target
  data_target_name <- paste0(prefix, '_data')
  data_target_sym <- rlang::sym(data_target_name)
  data_target <- tar_target_raw(data_target_name, substitute(data_loader()))
  ...
  fit_target_command <- substitute(eval(rlang::expr(
    meta_target_sym %>%
      rowwise() %>%
      mutate(
        enrichment = list(run_enrichment2(db, experiment, thresh, genesets, data_target_sym))
      ) %>%
      ungroup() %>%
      unnest(enrichment)
  )))
