...
#' fast multiplcation X %*% A where
#'  X = (X.sparse - outer(1, X.shift))
#' @param X.sparse a sparse nxp matrix
#' @params X.shift a p vector of column shifts
#' @param A the matrix by witch to multiple X
fastmatmulX = function(dat, A, left=TRUE){
  n <- dim(dat$X)[1]
  if(left){
    return(dat$X %*% A - outer(rep(1, n), (dat$X.shift%*%A)[1,]))
  } else{
    return(A %*% dat$X - sum(A) * dat$X.shift)
  }
}

#' fast multiplcation X^2 %*% A where
#'  X = (X.sparse - outer(1, X.shift))
#' @param X.sparse a sparse nxp matrix
#' @params X.shift a p vector of column shifts
#' @param A the matrix by witch to multiple X
#' NOTE: left=F only works with vectors now...
#' change to t(A), and colSums(A) + do type checking for vector case
fastmatmulX2 = function(dat, A, left=TRUE){
  n <- dim(dat$X)[1]
  if(left){
    return((dat$X^2 - 2*dat$X.scaled) %*% A + outer(rep(1, n), (dat$X.shift^2 %*% A)[1,]))
  } else{
    return(A %*% (dat$X^2 - 2*dat$X.scaled) + sum(A) * dat$X.shift^2)
  }
}
...
calc_Q = function(dat, Sigma2, Mu, Alpha, Z, delta) {
  ...
  tmp <- fastmatmulX2(dat, ASU2)
  Q = BiocGenerics::rowSums(tmp) # start w/ sum_l E_(ql)[(x_i' b_l)^2]

  # now add 2 sum_l sum_{k>l} (x_i' b_l_post)(x_i' b_k_post)
  b_post_mat = Mu * Alpha # ea:%ch column is posterior mean for b_l, p x L
  X_b_post = fastmatmulX(dat, b_post_mat) # [i, l] = x_i' b_l_post
  Q = Q + BiocGenerics::rowSums(X_b_post)^2 - BiocGenerics::rowSums(X_b_post^2)

  # now, add other terms with Z and delta
  Q = Q + as.numeric(2 * fastmatmulX(dat, rowSums(b_post_mat)) * (Z %*% delta)) + as.numeric((Z %*% delta)^2)
  return(Q)
}
...
update_b_l = function(dat, y, xi, prior_weights, V, Sigma2, Mu, Alpha, l, Z, delta) {
  # X is data matrix
  ...
  common_denoms = (1 / V) + fastmatmulX2(dat, g_xi_5_xi, F)


  # update Alpha[, l]
  #nums = t(X) %*% (y - .5 -  (g_xi_5_xi * (fastmatmulX(dat, b_post_not_l) + (Z %*% delta)))) # numerator in exp
  tmp <- (y - .5 -  (g_xi_5_xi * (fastmatmulX(dat, b_post_not_l) + (Z %*% delta))))[, 1]# numerator in exp
  nums = fastmatmulX(dat, tmp, F)[1,]  # t(X) %*% tmp
  Alpha[, l] = as.numeric(
    log(prior_weights) + (nums^2 / (2*common_denoms)) - (1/2)*log(common_denoms))
...
update_delta = function(dat, y, xi, Mu, Alpha, Z) {
  # X is data matrix
  # y is binary response
  # xi is lower-bound approximation parameters
  # Mu[j, l] is the posterior mean for b_l when entry j is selected, p x L
  ...
  ZtDZ = t(Z) %*% D %*% Z
  DXB = D %*% fastmatmulX(dat, rowSums(Mu * Alpha))
...
update_all = function(dat,
  ...
  if (intercept) { # if covariates and/or intercept
    delta = update_delta(dat, y, xi, Mu, Alpha, Z)
  }
  ...
  for (l in 1:L) {
    res_l = update_b_l(dat, y, xi, prior_weights, prior_vars[l], Sigma2, Mu, Alpha, l, Z, delta)
    Sigma2 = res_l$Sigma2
    Mu = res_l$Mu
    Alpha = res_l$Alpha
  }
...
logistic.susie.init = function(
  ...
  # TODO: broken for standardizing
  # for fast sparse matrix operations on centered matrix
  ...
  if(standardize){
    ...
    X.shift <- colMeans(X)
    X.scaled <- X
    X.scaled@x <- X.scaled@x * rep.int(X.shift, diff(X.scaled@p))
    X.se <- sqrt(sparseMatrixStats::colVars(X))
  }
  ...
  dat <- list(
    X=X, X.scaled=X.scaled, X.shift=X.shift, X.se=X.se,
...
logistic.susie.iteration = function(res, compute_elbo=T){
  ...
  post_info  <- update_all(
    dat, dat$y, post_info$xi, dat$prior_weights,
...
calc_ELBO = function(dat, Alpha, Mu, Sigma2, V, prior_weights, Z, delta, xi) {
  ...
  expected_log_lik = sum(log(g(xi)) + (dat$y - .5) * as.numeric(
    (fastmatmulX(dat, b_post) + (Z %*% delta))) - (xi / 2))
